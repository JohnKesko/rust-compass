...
export class IntentAnalyzer {
	...
	private inferProjectTypeFromCargo(content: string): ProjectType {
		...
		// Check dependencies
		if (
			nameLower.includes("nom") ||
			nameLower.includes("pest") ||
			nameLower.includes("lalrpop")
		) {
			return "parser";
		}
		...
		if (
			nameLower.includes("actix") ||
			nameLower.includes("axum") ||
			nameLower.includes("rocket") ||
			nameLower.includes("warp")
		) {
			return "web-server";
		}
		if (
			nameLower.includes("bevy") ||
			nameLower.includes("ggez") ||
			nameLower.includes("macroquad")
		) {
			return "game";
		}
	...
	analyzeFile(document: vscode.TextDocument): FileIntent {
		...
		// File name hints
		if (
			fileName.includes("lexer") ||
			fileName.includes("scanner") ||
			fileName.includes("tokenizer")
		) {
			return "lexer";
		}
		...
		// Content hints
		if (
			content.includes("struct token") ||
			content.includes("enum token") ||
			content.includes("tokenize") ||
			content.includes("lex(")
		) {
			return "lexer";
		}
		if (
			content.includes("parse(") ||
			content.includes("struct parser") ||
			content.includes("expect_token") ||
			content.includes("peek_token")
		) {
			return "parser";
		}
		if (
			content.includes("enum expr") ||
			content.includes("enum stmt") ||
			content.includes("struct ast") ||
	...
	analyzeBlock(document: vscode.TextDocument, position: vscode.Position): BlockIntent {
		...
		// Detect patterns
		if (
			context.includes(".chars()") ||
			context.includes(".bytes()") ||
			context.includes("char_indices") ||
			(context.includes(".next()") && context.includes("char"))
		) {
			return "iterating-chars";
		}

		if (
			context.includes("tokens.") ||
			context.includes("token_iter") ||
			fnName.includes("token") ||
			(context.includes(".peek()") && context.includes("token"))
		) {
			return "iterating-tokens";
		}

		if (
			context.includes("string::new()") ||
			context.includes(".push_str") ||
			context.includes(".push(") ||
			context.includes("format!")
		) {
			...
			while ((match = charsNextPattern.exec(text)) !== null) {
				...
				if (after.includes(".next()") && !after.includes(".peekable()")) {
					...
					moments.push({
						...
						suggestion:
							"Building a lexer? Consider .peekable() to look ahead without consuming",
			...
			while ((match = unwrapPattern.exec(text)) !== null) {
				...
				if (unwrapCount >= 2) {
					...
					moments.push({
						...
						suggestion:
							"Multiple .unwrap() calls - consider proper error handling with ? or match",
						ruleId: "unwrap-usage",
						severity: "hint",
