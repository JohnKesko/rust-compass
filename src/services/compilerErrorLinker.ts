...
export class CompilerErrorLinker implements vscode.Disposable {
    ...
    private _onErrorLinked = new vscode.EventEmitter<{ error: vscode.Diagnostic; rules: Rule[] }>();
    public readonly onErrorLinked = this._onErrorLinked.event;

    constructor(ruleEngine: RuleEngine) {
        this.ruleEngine = ruleEngine;
        this.diagnosticCollection = vscode.languages.createDiagnosticCollection('rust-compass-hints');
    ...
    private processDocumentDiagnostics(uri: vscode.Uri): void {
        ...
        const rustDiagnostics = diagnostics.filter(d =>
     ...
     */
    private checkForLinkableError(uri: vscode.Uri, diagnostic: vscode.Diagnostic): void {
