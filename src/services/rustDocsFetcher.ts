...
export class RustDocsFetcher {
    ...
    private _parseMethodDoc(html: string, url: string): FetchedDocContent | null {
        ...
        let signature = '';
        const signatureMatch = html.match(new RegExp(`method\\.${methodName}[^]*?<pre[^>]*class="[^"]*rust[^"]*"[^>]*>([^<]+)</pre>`, 'i'));
        if (signatureMatch) {
    ...
    private _parseTypeDoc(html: string, url: string): FetchedDocContent | null {
        ...
        let description = '';
        const mainDocMatch = html.match(/<div class="docblock item-decl">([^]*?)<\/div>/i) ||
    ...
    private _extractCodeExamples(html: string, startIndex: number): string[] {
        ...
        const codeBlockRegex = /<pre[^>]*class="[^"]*rust[^"]*"[^>]*><code[^>]*>([^]*?)<\/code><\/pre>/gi;
        let match;
        let count = 0;

        while ((match = codeBlockRegex.exec(relevantHtml)) !== null && count < 3) {
            const code = this._cleanHtml(match[1]);
            if (code.length > 20 && code.length < 1000) { // Skip tiny or huge examples
                examples.push(code);
                count++;
            }
        }
