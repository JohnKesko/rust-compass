...
import { intentAnalyzer, TeachableMoment } from '../services/intentAnalyzer';
...
export class SmartDiagnosticProvider implements vscode.Disposable {
    ...
    private async analyzeDocument(document: vscode.TextDocument) {
        ...
        for (const moment of moments) {
            ...
            diagnostic.code = {
                value: moment.ruleId,
                target: vscode.Uri.parse(`command:rust-compass.learnMore?${encodeURIComponent(JSON.stringify({ ruleId: moment.ruleId }))}`)
            };
