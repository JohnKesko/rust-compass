...
import type { ProjectContext, Rule, RuleEngine } from "../rules";
import { CargoAnalyzerService, type DetectedDependencies, PatternTracker } from "../services";
...
export class RustHoverProvider implements vscode.HoverProvider {
	...
	async provideHover(
		...
		const match = this.ruleEngine.findMatchAtPosition(document, position, this.getContext());
		...
		if (guide) {
			...
			const aiPrompt = encodeURIComponent(
				this.buildAIPrompt(rule, guide, document, position, deps),
			);
			links.push(
				`[🤖 Ask AI](command:rust-compass.askAI?${encodeURIComponent(JSON.stringify({ prompt: aiPrompt, ruleId: rule.id }))})`,
			);
			...
			const learnMoreArgs = encodeURIComponent(JSON.stringify({ ruleId: rule.id }));
			content.appendMarkdown(
				`[📚 Learn more](command:rust-compass.learnMore?${learnMoreArgs}) · `,
			);

			const aiPrompt = encodeURIComponent(
				`Explain this Rust pattern and when to use it: ${rule.rustTerm}`,
			);
			content.appendMarkdown(
				`[🤖 Ask AI](command:rust-compass.askAI?${encodeURIComponent(JSON.stringify({ prompt: aiPrompt, ruleId: rule.id }))})`,
			);
...
Based on my specific code and project setup, which approach would be best and why? Keep the explanation beginner-friendly.`;
	 ...
	 */
	private getDecisionGuide(
		ruleId: string,
		inLoop: boolean,
		_deps: DetectedDependencies,
	): DecisionGuide | null {
		const guides: Record<string, DecisionGuide> = {
			...
			"collect-turbofish": {
				...
				thinkAbout:
					"Use type annotation or turbofish: let v: Vec<_> = ... or .collect::<Vec<_>>()",
				rustBookLink:
				...
				depAware: (d) => {
					...
					if (d.async.length > 0) {
						extra.push({
							method: ".collect::<Result<Vec<_>, _>>()",
							useWhen: "collecting fallible async results",
						});
					}
					if (d.serialization.includes("serde")) {
						extra.push({
							method: "serde_json::Value",
							useWhen: "collecting into JSON structure",
						});
					}
					return extra;
				},
			...
			"string-vs-str": {
				...
				alternatives: [
					...
					{
						method: "String",
						useWhen: "owned, need to modify, store in struct",
					},
				...
				depAware: (d) => {
					...
					if (d.error.includes("anyhow")) {
						extra.push({
							...
							useWhen: "easy error propagation with context",
						});
						extra.push({
							method: '.context("msg")',
							useWhen: "add context to errors (anyhow)",
						});
					}
					if (d.error.includes("thiserror")) {
						extra.push({
							method: "#[derive(Error)]",
							useWhen: "define custom error types",
						});
					}
					if (d.error.includes("eyre") || d.error.includes("color-eyre")) {
						extra.push({
							method: "eyre::Result",
							useWhen: "rich error reports with color-eyre",
						});
					}
					return extra;
				},
			...
			"expect-usage": {
				...
				alternatives: [
					{
						method: '.expect("reason")',
						useWhen: 'panic should "never" happen',
					},
					{ method: "?", useWhen: "caller should handle the error" },
					{
						method: ".unwrap_or_else(|| ...)",
						useWhen: "compute fallback lazily",
					},
				],
			...
			"lexer-peekable-chars": {
				...
				depAware: (d) => {
					...
					if (d.parsing.includes("nom")) {
						extra.push({
							method: "nom combinators",
							useWhen: "you have nom — use its parser combinators instead",
						});
					}
					if (d.parsing.includes("pest")) {
						extra.push({
							method: "pest grammar",
							useWhen: "you have pest — define grammar in .pest file",
						});
					}
					if (d.parsing.includes("logos")) {
						extra.push({
							method: "#[derive(Logos)]",
							useWhen: "you have logos — use derive macro for lexer",
						});
					}
					if (d.parsing.includes("chumsky")) {
						extra.push({
							method: "chumsky parsers",
							useWhen: "you have chumsky — use its combinator API",
						});
					}
					return extra;
				},
			...
			"match-exhaustive": {
				...
				alternatives: [
					{
						method: "explicit arms",
						useWhen: "handle each variant differently",
					},
