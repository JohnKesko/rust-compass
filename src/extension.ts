...
import { RuleEngine, ProjectContext } from './rules';
import { RustHoverProvider, RustDecorationProvider, RustCodeActionProvider, SmartDiagnosticProvider } from './providers';
...
export function activate(context: vscode.ExtensionContext) {
	...
	cargoAnalyzer.initialize().then(async () => {
		...
		if (suggestion) {
			...
			if (!currentSetting || currentSetting === 'general') {
				...
				if (choice === 'Yes') {
					await config.update('projectContext', suggestion.context, vscode.ConfigurationTarget.Workspace);
					currentContext = suggestion.context as ProjectContext;
					vscode.window.showInformationMessage(`Project context set to: ${suggestion.context}`);
				}
			}
		}
	});
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('rust-compass.showProgress', async () => {
			...
			const items: Array<{ label: string; description: string; ruleId?: string }> = [
				{ label: '📊 Session Stats', description: `${session.totalPatterns} patterns (${session.uniquePatterns} unique)` },
				{ label: '📈 All Time', description: `${allTime.totalPatterns} total patterns seen` },
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('rust-compass.restoreAllHints', async () => {
			...
			const choice = await vscode.window.showQuickPick(
				[
					{ label: 'Restore All', description: `Restore all ${dismissed.length} hidden hints`, value: 'all' },
					...dismissed.map(id => {
			...
			if (choice) {
				if (choice.value === 'all') {
					...
					vscode.window.showInformationMessage(`Restored ${dismissed.length} hidden hints.`);
				} else {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('rust-compass.setProjectContext', async () => {
			...
			if (selected) {
				...
				const config = vscode.workspace.getConfiguration('rustCompass');
				await config.update('projectContext', selected.value, vscode.ConfigurationTarget.Workspace);
				vscode.window.showInformationMessage(
					`Project context set to: ${selected.label}`
				);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('rust-compass.askAI', async (args: { prompt: string; ruleId: string }) => {
	...
	context.subscriptions.push(
		vscode.workspace.onDidChangeTextDocument(event => {
			...
			if (editor && event.document === editor.document && editor.document.languageId === 'rust') {
				decorationProvider.triggerUpdate(editor);
			}
		})
	);
